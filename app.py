...
def build_diagnosis_pdf(
    ...
    RED    = colors.HexColor("#E8593C")
    GREEN  = colors.HexColor("#1D9E75")
    AMBER  = colors.HexColor("#BA7517")
    PURPLE = colors.HexColor("#534AB7")
    CLASS_PDF_COLORS = {
        "Healthy": GREEN,
        "Diabetes / Prediabetes": RED,
        "High Cardiovascular Risk": AMBER,
        "Cardiometabolic Risk": PURPLE,
    }
    result_color = CLASS_PDF_COLORS.get(label, NAVY)
    ...
    s_title   = sty("title",   fontSize=20, textColor=colors.white, leading=24, spaceAfter=0, fontName="Helvetica-Bold")
    s_sub     = sty("sub",     fontSize=9,  textColor=colors.HexColor("#C8D6E8"), leading=13, fontName="Helvetica")
    s_sect    = sty("sect",    fontSize=8,  textColor=BLUE, fontName="Helvetica-Bold",
                    spaceBefore=10, spaceAfter=4, letterSpacing=1.2)
    s_diag    = sty("diag",    fontSize=22, textColor=result_color, fontName="Helvetica-Bold",
                    spaceBefore=4, spaceAfter=2, leading=26)
    s_conf    = sty("conf",    fontSize=10, textColor=GREY, fontName="Helvetica", spaceAfter=8)
    s_body    = sty("body",    fontSize=9,  textColor=NAVY, leading=14, fontName="Helvetica", spaceAfter=4)
    s_small   = sty("small",   fontSize=7.5, textColor=GREY, leading=12, fontName="Helvetica")
    s_footer  = sty("footer",  fontSize=7,  textColor=GREY, alignment=TA_CENTER, fontName="Helvetica-Oblique")
    s_meta    = sty("meta",    fontSize=9,  textColor=GREY, alignment=TA_RIGHT, fontName="Helvetica")
    ...
    header_data = [[
        ...
        Paragraph(f"Generated {datetime.now().strftime('%d %b %Y  %H:%M')}", s_meta),
    ]]
    ...
    story.append(Paragraph("Federated AI — Chronic Disease Risk Report", s_sub))
    story.append(Spacer(1, 4*mm))
    story.append(hr())

    # ── Patient info ─────────────────────────────────────────────────
    story.append(section("Patient"))
    info_data = [
        ["Age", str(age_years), "Sex", sex_label],
    ]
    info_tbl = Table(info_data, colWidths=["18%", "32%", "18%", "32%"])
    ...
    story.append(Paragraph(label, s_diag))
    story.append(Paragraph(f"Model confidence: {confidence*100:.1f}%", s_conf))

    prob_rows = [["Condition", "Probability"]]
    for cname, p in zip(CLASS_NAMES.values(), proba):
        prob_rows.append([cname, f"{p*100:.1f}%"])
    prob_tbl = Table(prob_rows, colWidths=["72%", "28%"])
    ...
    story.append(section("Top Feature Contributions"))
    feat_rows = [["Feature", "Score", "Direction"]]
    for name, val in zip(feat_names[:10], feat_vals[:10]):
        direction = "Supports prediction" if val > 0 else "Suggests other condition"
        feat_rows.append([name, f"{val:+.3f}", direction])
    feat_tbl = Table(feat_rows, colWidths=["44%", "18%", "38%"])
    ...
    # ── Clinical summary ─────────────────────────────────────────────
    story.append(section("Clinical Summary"))
    ...
    if gender_flags:
        story.append(hr())
        story.append(section("Additional Risk Factors"))
        for flag_label, flag_note, risk_level in gender_flags:
            flag_color = colors.HexColor(FLAG_COLORS.get(risk_level, "#64748B"))
            flag_data = [[
                Paragraph(f"<b>{flag_label}</b><br/>{flag_note}", s_small)
            ]]
            flag_tbl = Table(flag_data, colWidths=["100%"])
            flag_tbl.setStyle(TableStyle([
                ("LEFTPADDING",   (0, 0), (-1, -1), 8),
                ("TOPPADDING",    (0, 0), (-1, -1), 5),
                ("BOTTOMPADDING", (0, 0), (-1, -1), 5),
                ("LINEBEFORE",    (0, 0), (0, -1), 3, flag_color),
    ...
    story.append(Paragraph(
        "Decision-support tool only. This report does not constitute a medical diagnosis. "
        "Clinical judgment is required for all patient care decisions. "
    ...
    if not model_ready:
        ...
        with col2:
            ...
            if st.session_state.get("prediction_done"):
                ...
                st.markdown("<div class='result-section-label' style='margin-top:20px'>Download Report</div>",
                            unsafe_allow_html=True)
                try:
                    _gender_flag_tuples = [
                        (lbl, note, lvl)
                        for _, (lbl, note, lvl) in active_gender_flags
                    ]
                    pdf_bytes = build_diagnosis_pdf(
                        label=label,
                        confidence=confidence,
                        proba=proba.tolist(),
                        feat_names=feat_names_top,
                        feat_vals=feat_vals,
                        report_text=report,
                        gender_flags=_gender_flag_tuples,
                        age_years=age_years,
                        sex_label=sex_label,
                    )
                    st.download_button(
                        label="Download PDF",
                        data=pdf_bytes,
                        file_name="fedxai_diagnosis.pdf",
